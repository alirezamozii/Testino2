...
export class SqliteWorkerClient implements DatabasePort {
  ...
  async open(ownerKey = "default") {
    ...
    if (hasWebLocks && hasChannel) {
      // A same-tab reload can overlap briefly with teardown of the previous
      // document. Give that lock time to disappear before treating this page
      // as a genuine second tab.
      let gotLeaderLock = false;
      for (let attempt = 0; attempt < 20 && !gotLeaderLock; attempt += 1) {
        gotLeaderLock = await this.tryAcquireLeaderLock(ownerKey);
        if (!gotLeaderLock && attempt < 19) {
      ...
      // Fallback for single-tab / environments without locks
      this.isLeader = true;

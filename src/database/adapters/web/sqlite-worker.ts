...
async function openDatabase(ownerKey = "default") {
  ...
  if (typeof navigator === "undefined" || !("storage" in navigator) || !("getDirectory" in navigator.storage)) {
    throw new Error("مرورگر شما ذخیره‌سازی پایدار OPFS را پشتیبانی نمی‌کند. لطفاً از مرورگر سازگار (Chrome/Edge/Firefox) استفاده کنید.");
  }
  ...
  database.exec({ sql: "PRAGMA foreign_keys=ON; PRAGMA busy_timeout=5000;" });
}
...
self.onmessage = async (event: MessageEvent<DatabaseRequest>) => {
  ...
  try {
    if (request.type === "open") {
      ...
      reply = { id: request.id, ok: true, storage: "opfs" };
      ...
      reply = { id: request.id, ok: true, storage: "opfs", schemaVersion: schemaVer };

    ...
    <div className="space-y-6 max-w-6xl mx-auto pb-10">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        <div className="lg:col-span-7 space-y-6">
          ...
          {/* 3b. Goals & Study Plan — slim entry card (the /goals page is feature-rich but
              secondary, so it lives outside the bottom nav and gets this dedicated entry) */}
          <Link
            href="/goals/"
            className="quick-action-card quick-card-primary !bg-[var(--surface)] !text-[var(--ink)] border-2 border-[var(--line-strong)] flex-row items-center justify-between gap-3 py-3.5 px-4 shadow-[3px_3px_0px_var(--neo-shadow)] hover:translate-x-[1px] hover:translate-y-[1px] transition-all"
            aria-label="اهداف و برنامه مطالعه"
          >
            <div className="flex items-center gap-3 min-w-0">
              <div className="w-10 h-10 rounded-2xl bg-[var(--brand-orange)] border-2 border-[var(--line-strong)] flex items-center justify-center text-white shadow-[1.5px_1.5px_0px_var(--neo-shadow)] shrink-0">
                <Target size={20} strokeWidth={2.5} />
              </div>
              <div className="min-w-0">
                <strong className="text-sm font-black block text-[var(--ink)]">اهداف و برنامهٔ مطالعه</strong>
                <span className="text-[11px] font-bold text-[var(--muted)] block mt-0.5 truncate">
                  هدف بگذار، مسیر گام‌به‌گام را ببین
                </span>
              </div>
            </div>
            <ChevronLeft size={18} className="text-[var(--brand-orange)] shrink-0" />
          </Link>
